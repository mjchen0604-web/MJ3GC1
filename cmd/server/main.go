...
import (
	...
	mj3gcaccess "github.com/router-for-me/CLIProxyAPI/v6/internal/access/mj3gc_access"
...
func main() {
	fmt.Printf("MJ3GC Version: %s, Commit: %s, BuiltAt: %s\n", buildinfo.Version, buildinfo.Commit, buildinfo.BuildDate)
	...
	log.Infof("MJ3GC Version: %s, Commit: %s, BuiltAt: %s", buildinfo.Version, buildinfo.Commit, buildinfo.BuildDate)
	...
	mj3gc.EnsureAccessProvider(cfg)
	mj3gcStore := mj3gc.DefaultStore()
	mj3gcStore.SetPath(mj3gc.ResolveDataPath(cfg, configFilePath))
	if err := mj3gcStore.Load(); err != nil {
		log.WithError(err).Warn("failed to load mj3gc data store")
	}
